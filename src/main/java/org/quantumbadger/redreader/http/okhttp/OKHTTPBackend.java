...
public class OKHTTPBackend extends HTTPBackend {
	...
	private OKHTTPBackend() {
		...
		OkHttpClient temp = builder.build();
		mClient = PrefetchingLib.getOkHttp(temp);
	...
	public Request prepareRequest(final Context context, final RequestDetails details) {
		...
		return new Request() {
			...
			public void executeInThisThread(final Listener listener) {
				...
				Log.e("PREFETCHING_LIB_REQUEST", "Time: "+ Calendar.getInstance().getTimeInMillis());
				final Call call = mClient.newCall(builder.build());
				Log.e("OK", "calling: " + call.request().url());
				callRef.set(call);
				...
				try {
					...
					Log.e("PREFETCHING_LIB_RESP", "Time: "+ Calendar.getInstance().getTimeInMillis());
				} catch(IOException e) {
					Log.e("PREFETCHING_LIB_RESP", "FAIL");
